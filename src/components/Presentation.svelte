...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8 my-18">
  ...
  <section class="flex flex-col justify-center">
    ...
    <h2 class="text-2xl italic">{$t('freelance_full_stack_developer')}</h2>
    <section class="my-8">
      <p class="my-3">
        {$t('currently')}
        {$t('freelance_full_stack_developer')}
        <Link href="https://twitter.com/kiabifrance" title={$t('title.profile', { values: { name: 'Kiabi Twitter' } })}>
          @Kiabi
        </Link>
        {$t('in')}
        <Link href="https://www.google.com/maps/search/?api=1&query=Hem%2C%20France" title="Hem {$t('on')} Google Map">
          Hem, France
        </Link>
      </p>
      <p class="my-3">
